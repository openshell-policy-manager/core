...
if [ -d "$HOME/.config/opencode/gsd-local-patches" ]; then
  ...
  PATCHES_DIR="/Users/tkopatz/dev/work/tk-repos/openshell-policy-manager/.opencode/gsd-local-patches"
fi
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `/Users/tkopatz/dev/work/tk-repos/openshell-policy-manager/.opencode/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
